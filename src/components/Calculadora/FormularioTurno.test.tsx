...
describe('FormularioTurno', () => {
  ...
  const defaultProps = {
    ...
    jornadaValida: true,
    tipoJornada: 'estandar' as const,
    onTipoJornadaChange: vi.fn(),
    diasDescanso: [0] as number[],
    onDiasDescansoChange: vi.fn(),
  };

  it('renderiza título y campo de fecha', () => {
    render(<FormularioTurno {...defaultProps} />);
  ...
  it('llama onFechaChange al cambiar fecha', () => {
    ...
    render(<FormularioTurno {...defaultProps} onFechaChange={onFechaChange} />);
  ...
  it('llama onFranjaChange al cambiar hora de franja', () => {
    ...
    render(<FormularioTurno {...defaultProps} onFranjaChange={onFranjaChange} />);
  ...
  it('oculta botón Añadir franja cuando hay 4', () => {
    ...
    render(<FormularioTurno {...defaultProps} franjas={franjas} turno={{ fecha: defaultTurno.fecha, franjas }} />);
  ...
  it('llama onAgregarFranja al hacer click en añadir', () => {
    ...
    render(<FormularioTurno {...defaultProps} onAgregarFranja={onAgregarFranja} />);
  ...
  it('deshabilita botón Calcular cuando jornada no es válida', () => {
    render(<FormularioTurno {...defaultProps} jornadaValida={false} />);
  ...
  it('llama onCalcular al presionar Calcular', async () => {
    const onCalcular = vi.fn();

    render(<FormularioTurno {...defaultProps} onCalcular={onCalcular} />);

    fireEvent.click(screen.getByText('Calcular'));
    await waitFor(() => expect(onCalcular).toHaveBeenCalled());
  });
  ...
  it('muestra alert de cruce medianoche cuando fin < inicio', () => {
    ...
    render(<FormularioTurno {...defaultProps} franjas={franjas} turno={turno} />);
